        ...
        (void) vocab::tuple_for_each(
            [&](auto field) {
                ...
                (void) vformat_encoded_context<meta::Encoding<decltype(context)>>(
                    u8"{}"_sv, fmt::make_format_args<decltype(context)>(field.get(value)), context, true);
            },
            reflection_value);
...
constexpr auto tag_invoke(types::Tag<formatter_in_place>, InPlaceType<T>, FormatParseContext<Enc>& parse_context,
    ...
    return fmt::formatter<R, Enc>(parse_context, debug) % [](concepts::CopyConstructible auto formatter) {
